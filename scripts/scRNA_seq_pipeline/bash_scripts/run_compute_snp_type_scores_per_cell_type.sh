...
##SBATCH --array=1-3 # specify if more than one dataset being analyzed
...
output_dir=/ix/djishnu/Priyamvada/RNA_seq_GWAS_integrated_analysis/example/plots_output/snp_score_barplots
...
##Script
Rscript --no-save --no-restore --verbose /ix/djishnu/Priyamvada/RNA_seq_GWAS_integrated_analysis/R_scripts/Compute_snp_type_scores_for_cell_types.R $SNP_dir $SNP_dataset $SNP_file $phenotype $RNA_seq_dir $RNA_seq_folder $output_dir $snp_type > /ix/djishnu/Priyamvada/RNA_seq_GWAS_integrated_analysis/test.Rout 2>&1
